...
- Hardened security via `_headers` (CSP, Permissions-Policy, nosniff, frame-ancestors none) and documented required environment variables for Cloudflare Pages; `/admin` uses a scoped CSP that permits `unsafe-eval` for Decap while the public site stays strict.
